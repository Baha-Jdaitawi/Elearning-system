...
export const createCourseController = asyncHandler(async (req, res) => {
  ...
  if (!title || !description || !category_id) {
    throw new AppError('Title, description, and category are required', HTTP_STATUS.BAD_REQUEST);
  }
...
export const updateCourseController = asyncHandler(async (req, res) => {
  ...
  if (title) {
    // ✅ FIXED - Correct parameter order: instructorId, title, excludeId
    const titleExists = await courseTitleExistsForInstructor(course.instructor_id, title, parseInt(id));
    if (titleExists) {
