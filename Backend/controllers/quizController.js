...
export const getQuizzesByLessonController = asyncHandler(async (req, res) => {
  ...
  if (forStudent === 'true' && user.role === USER_ROLES.STUDENT) {
    // Students get questions only (no answers) - enrollment already checked in middleware
...
export const getRandomQuizQuestionsController = asyncHandler(async (req, res) => {
  ...
  // For students, enrollment check would be handled by middleware if needed
  // For now, allowing access to random questions for practice
